...
import api from '../../api/api'; // <--- Import existing API instance

const publicKey = 'public_4c8twCaRhq67pmSzubBwraeuJvQ=';
const urlEndpoint = 'https://ik.imagekit.io/wjmididvj';

// Authentication Function using our global API instance
const authenticator = async () => {
    try {
        // api.get uses the correct Render/Local URL automatically
        const response = await api.get('/imagekit/auth');
        const { signature, expire, token } = response.data;
        return { signature, expire, token };
    } catch (error) {
        console.error("ImageKit Auth Error:", error.response?.data || error.message);
        ...
        >
            <div className="w-full">
                <div className={`relative border-2 border-dashed rounded-lg p-3 transition-all duration-300 
                    ...
                    {error && <p className="text-[9px] text-red-500 mt-1 text-center font-medium">{error}</p>}
                </div>
            </div>
        </IKContext>
    );
